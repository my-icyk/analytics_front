...
export type AdminView = 'overview' | 'users' | 'user' | 'roles' | 'role' | 'permissions'
...
export type AdminPageDefinition = {
  id: AdminView | 'counters' | 'groups' | 'rules'
  label: string
  icon: LucideIcon
  group?: string
...
export function parseAdminRoute(search: string): AdminRouteState {
  ...
  if (view === 'users' || view === 'permissions') return { view, userId: null, roleId: null }
  if (view === 'user') return { view: 'user', userId, roleId: null }
  if (view === 'roles') return { view: 'roles', userId: null, roleId: null }
  if (view === 'role') return { view: 'role', userId: null, roleId }
  return { view: 'overview', userId: null, roleId: null }
}

export function buildAdminRoute(view: AdminView | 'counters' | 'groups' | 'rules', params: { userId?: number | null; roleId?: number | null } = {}) {
